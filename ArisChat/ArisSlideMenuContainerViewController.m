//
//  ArisSlideMenuContainerViewController.m
//  ArisChat
//
...
typedef enum {
    ArisSLideMenuPanDirectionNone,
    ArisSLideMenuPanDirectionLeft,
    ArisSLideMenuPanDirectionRight
} ArisSLideMenuPanDirection;

@interface ArisSlideMenuContainerViewController ()
@property (nonatomic, strong) UIView *menuContainerView;

@property (nonatomic, assign) CGPoint panGestureOrigin;
@property (nonatomic, assign) CGFloat panGestureVelocity;
@property (nonatomic, assign) ArisSLideMenuPanDirection panDirection;

@property (nonatomic, assign) BOOL viewHasAppeared;
@end

@implementation ArisSlideMenuContainerViewController
...
+ (ArisSlideMenuContainerViewController *)containerWithCenterViewController:(id)centerViewController
    ...
    ArisSlideMenuContainerViewController *controller = [ArisSlideMenuContainerViewController new];
...
- (void)setDefaultSettings {
    ...
    self.menuState = ArisSLideMenuStateClosed;
    ...
    self.panMode = ArisSLideMenuPanModeDefault;
...
- (void)toggleLeftSideMenuCompletion:(void (^)(void))completion {
    if(self.menuState == ArisSLideMenuStateLeftMenuOpen) {
        [self setMenuState:ArisSLideMenuStateClosed completion:completion];
    } else {
        [self setMenuState:ArisSLideMenuStateLeftMenuOpen completion:completion];
    }
}

- (void)toggleRightSideMenuCompletion:(void (^)(void))completion {
    if(self.menuState == ArisSLideMenuStateRightMenuOpen) {
        [self setMenuState:ArisSLideMenuStateClosed completion:completion];
    } else {
        [self setMenuState:ArisSLideMenuStateRightMenuOpen completion:completion];
    }
}
...
- (void)setMenuState:(ArisSLideMenuState)menuState {
...
- (void)setMenuState:(ArisSLideMenuState)menuState completion:(void (^)(void))completion {
    void (^innerCompletion)() = ^ {
        ...
        ArisSLideMenuStateEvent eventType = (_menuState == ArisSLideMenuStateClosed) ? ArisSLideMenuStateEventMenuDidClose : ArisSLideMenuStateEventMenuDidOpen;
    ...
    switch (menuState) {
        case ArisSLideMenuStateClosed: {
            [self sendStateEventNotification:ArisSLideMenuStateEventMenuWillClose];
        ...
        case ArisSLideMenuStateLeftMenuOpen:
            if(!self.leftMenuViewController) return;
            [self sendStateEventNotification:ArisSLideMenuStateEventMenuWillOpen];
            [self leftMenuWillShow];
            [self openLeftSideMenuCompletion:innerCompletion];
            break;
        case ArisSLideMenuStateRightMenuOpen:
            if(!self.rightMenuViewController) return;
            [self sendStateEventNotification:ArisSLideMenuStateEventMenuWillOpen];
...
- (void)sendStateEventNotification:(ArisSLideMenuStateEvent)event {
    NSDictionary *userInfo = @{@"eventType": [NSNumber numberWithInt:event]};
    [[NSNotificationCenter defaultCenter] postNotificationName:ArisSlideMenuStateNotificationEvent
                                                        object:self
                                                      userInfo:userInfo];
}
...
- (BOOL) sideMenuPanEnabled {
    return ((self.panMode & ArisSLideMenuPanModeSideMenu) == ArisSLideMenuPanModeSideMenu);
}
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch {
    if([gestureRecognizer isKindOfClass:[UITapGestureRecognizer class]] &&
       self.menuState != ArisSLideMenuStateClosed) return YES;
...
- (void) handlePan:(UIPanGestureRecognizer *)recognizer {
	...
	if(recognizer.state == UIGestureRecognizerStateBegan) {
        ...
        self.panDirection = ArisSLideMenuPanDirectionNone;
	}
    
    if(self.panDirection == ArisSLideMenuPanDirectionNone) {
        CGPoint translatedPoint = [recognizer translationInView:view];
        if(translatedPoint.x > 0) {
            self.panDirection = ArisSLideMenuPanDirectionRight;
            if(self.leftMenuViewController && self.menuState == ArisSLideMenuStateClosed) {
                [self leftMenuWillShow];
            }
        }
        else if(translatedPoint.x < 0) {
            self.panDirection = ArisSLideMenuPanDirectionLeft;
            if(self.rightMenuViewController && self.menuState == ArisSLideMenuStateClosed) {
                [self rightMenuWillShow];
            }
        }
    }
    
    if((self.menuState == ArisSLideMenuStateRightMenuOpen && self.panDirection == ArisSLideMenuPanDirectionLeft)
       || (self.menuState == ArisSLideMenuStateLeftMenuOpen && self.panDirection == ArisSLideMenuPanDirectionRight)) {
        self.panDirection = ArisSLideMenuPanDirectionNone;
        return;
    }
    
    if(self.panDirection == ArisSLideMenuPanDirectionLeft) {
        [self handleLeftPan:recognizer];
    } else if(self.panDirection == ArisSLideMenuPanDirectionRight) {
        [self handleRightPan:recognizer];
    }
}

- (void) handleRightPan:(UIPanGestureRecognizer *)recognizer {
    if(!self.leftMenuViewController && self.menuState == ArisSLideMenuStateClosed) return;
    ...
    if(recognizer.state == UIGestureRecognizerStateEnded) {
        ...
        if(self.menuState == ArisSLideMenuStateClosed) {
            ...
            if(hideMenu) {
                self.panGestureVelocity = velocity.x;
                [self setMenuState:ArisSLideMenuStateClosed];
            } else {
                self.panGestureVelocity = 0;
...
- (void) handleLeftPan:(UIPanGestureRecognizer *)recognizer {
    if(!self.rightMenuViewController && self.menuState == ArisSLideMenuStateClosed) return;
	...
	if(recognizer.state == UIGestureRecognizerStateEnded) {
        ...
        if(self.menuState == ArisSLideMenuStateClosed) {
            ...
            if(showMenu) {
                self.panGestureVelocity = velocity.x;
                [self setMenuState:ArisSLideMenuStateRightMenuOpen];
            } else {
                self.panGestureVelocity = 0;
                [self setCenterViewControllerOffset:0 animated:YES completion:nil];
            }
...
- (void)setUserInteractionStateForCenterViewController {
    ...
    if([self.centerViewController respondsToSelector:@selector(viewControllers)]) {
        ...
        for(UIViewController* viewController in viewControllers) {
            viewController.view.userInteractionEnabled = (self.menuState == ArisSLideMenuStateClosed);
        }
    }
}
